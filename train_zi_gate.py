...
# ====================================================================
class ZeroInflatedGate:
    """
    Two-stage gating network:
      Stage 1: Zero-regime detector (threshold on tft_pred)
      Stage 2: LightGBM regressor predicting error difference on non-zero regime
    """
    def __init__(self, zero_threshold=0.5, temperature=0.5):
    ...
    def _prepare_features(self, merged_df, ctx_cols):
        """Build feature matrix with dead features removed and new features added."""
        # Remove dead features (zero variance)
        live_features = [c for c in ctx_cols if merged_df[c].std() > 1e-8]
        
        # Add derived features
        merged_df["lgb_minus_tft"] = merged_df["lgb_pred"] - merged_df["tft_pred"]
        
        self.feature_names = live_features + ["lgb_minus_tft", "item_idx", "store_idx"]
        return merged_df[self.feature_names].values, self.feature_names
    
    def train(self, val_merged, ctx_cols, denominators):
        """Train the zero-inflated gating network on validation data."""
